...
public class ExplodeBarrelController : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        GameObject effect = Instantiate(particleExplode, transform.position, transform.rotation);﻿using System.Collections;
using System.Collections.Generic;
using UnityEngine;
...
public class ExplodeBarrelController : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        foreach (Collider2D c in damage)
        {
            ...
            if (rb2d != null)
            {
                if (!c.gameObject.CompareTag("World"))
                {
                	...
                	/* TODO
                		Baradoros:
                		raycast towards c.gameObject to get distance. 
                		feed distance into linear formula at first, log if needed. 
                		apply force away from explosion as impulse
        ...
        foreach (Collider2D c in damage)
        {
            Rigidbody2D rb2d = c.GetComponent<Rigidbody2D>();
            //Checking that object can be move
            if (rb2d != null)﻿using System.Collections;
using System.Collections.Generic;
using UnityEngine;
